...
- **.23** (`2a09:3f00:2004::1`): `mini3` - Serves as the stable IPv4 NAT Gateway (routing traffic out through WireGuard), Default IPv6 Gateway (via WireGuard Tunnel), DNS Server (Pi-Hole), and Reverse Proxy (Apache).
...
---
**WireGuard Note**: To use Pi-hole DNS over the VPN tunnel, add `DNS = 192.168.1.23` (or the VPN-IP `10.2.3.9` / `10.5.0.1`) to your client config.
